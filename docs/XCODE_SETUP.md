...
- The Family Controls entitlement on `Grasstouch` and `GrasstouchMonitor` requires Apple to grant the distribution entitlement to your team — request it at <https://developer.apple.com/contact/request/family-controls-distribution> before TestFlight.
...
- **Tunnel runs but speed isn't capped** → `GrasstouchMonitor` isn't firing `eventDidReachThreshold`. Add an `os_log` line and check the device console. Without it, `throttleActive` stays false and the tunnel passes through at line rate.
